...
class SetGraph:
    def __init__(self, vacancies_salary, vacancies_count, profes_salary, profes_count, cities_procent, cities_data,
                 profes_name):
    ...
    def create_salary_graph(self):
        ...
        self.axes[0, 0].bar(self.o_x + self.width / 2, self.prof_salary.values(), self.width, label=f'З/п: {self.profession.lower()}')
    ...
    def create_vacancy_count_graph(self):
        self.axes[0, 1].bar(self.o_x - self.width / 2, self.vacancies_count.values(), self.width, label='Количество вакансий')
        self.axes[0, 1].bar(self.o_x + self.width / 2, self.prof_count.values(), self.width, label=f'Кол-во вакансий: {self.profession.lower()}')
...
vac_data, prof_data, city_procent_table, city_data_table = input_conect.data.get_table_data()
exel = Report(input_profession, vac_data, prof_data, city_procent_table, city_data_table)
exel.generate_excel()
vac_salary, vac_count,  prof_salary, prof_count, city_procent, city_data = input_conect.data.get_statistics_data()
